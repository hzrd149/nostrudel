...
// node_modules/.pnpm/nostr-wasm@0.1.0/node_modules/nostr-wasm/dist/nostr.js
...
var map_wasm_imports = (g_imports) => ({
  a: {
    a: g_imports.abort,
    f: g_imports.memcpy,
    d: g_imports.resize,
...
var map_wasm_exports = (g_exports) => ({
  malloc: g_exports["i"],
  free: g_exports["j"],
...
var NostrWasm = async (z_src) => {
  ...
  g_wasm.init();
  const ip_sk = g_wasm.malloc(
    32
    /* ByteLens.PRIVATE_KEY */
  );
  const ip_ent = g_wasm.malloc(
    32
    /* ByteLens.NONCE_ENTROPY */
  );
  const ip_msg_hash = g_wasm.malloc(
    32
    /* ByteLens.MSG_HASH */
  );
  const ip_pubkey_scratch = g_wasm.malloc(
    32
    /* ByteLens.XONLY_PUBKEY */
  );
  const ip_sig_scratch = g_wasm.malloc(
    64
    /* ByteLens.BIP340_SIG */
  );
  const ip_keypair = g_wasm.malloc(
    96
    /* ByteLens.KEYPAIR_LIB */
  );
  const ip_xonly_pubkey = g_wasm.malloc(
    64
    /* ByteLens.XONLY_KEY_LIB */
  );
  const ip_sha256 = g_wasm.malloc(
    104
    /* ByteLens.SHA256_LIB */
  );
  const ip_ctx = g_wasm.context_create(
    513 | 257
    /* Flags.CONTEXT_VERIFY */
  );
  ...
  return {
    generateSecretKey: () => crypto.getRandomValues(new Uint8Array(
      32
      /* ByteLens.PRIVATE_KEY */
    )),
    getPublicKey(sk) {
      if (1 !== with_keypair(sk, () => g_wasm.keypair_xonly_pub(ip_ctx, ip_xonly_pubkey, null, ip_keypair))) {
        throw Error("failed to get pubkey from keypair");
      }
      g_wasm.xonly_pubkey_serialize(ip_ctx, ip_pubkey_scratch, ip_xonly_pubkey);
      return ATU8_HEAP.slice(
        ip_pubkey_scratch,
        ip_pubkey_scratch + 32
        /* ByteLens.XONLY_PUBKEY */
      );
    },
    finalizeEvent(event, seckey, ent) {
      with_keypair(seckey, () => {
        ...
        if (1 !== g_wasm.schnorrsig_sign32(ip_ctx, ip_sig_scratch, ip_msg_hash, ip_keypair, ip_ent)) {
    ...
    verifyEvent(event) {
      ...
      if (1 !== g_wasm.xonly_pubkey_parse(ip_ctx, ip_xonly_pubkey, ip_pubkey_scratch)) {
        throw Error("pubkey is invalid");
      }
      if (1 !== g_wasm.schnorrsig_verify(ip_ctx, ip_sig_scratch, ip_msg_hash, 32, ip_xonly_pubkey)) {
