...
const NOSTR_IDB_NAME = "nostr-idb";
const NOSTR_IDB_VERSION = 2;
async function openDB(name = NOSTR_IDB_NAME, callbacks) {
    return await openDB$1(name, NOSTR_IDB_VERSION, {
        ...callbacks,
...
__export(kinds_exports, {
  ...
  isParameterizedReplaceableKind: () => isParameterizedReplaceableKind$1,
  isRegularKind: () => isRegularKind,
  isReplaceableKind: () => isReplaceableKind$1
});
function isRegularKind(kind) {
  return 1e3 <= kind && kind < 1e4 || [1, 2, 4, 5, 6, 7, 8, 16, 40, 41, 42, 43, 44].includes(kind);
}
function isReplaceableKind$1(kind) {
  return [0, 3].includes(kind) || 1e4 <= kind && kind < 2e4;
}
function isEphemeralKind(kind) {
  return 2e4 <= kind && kind < 3e4;
}
function isParameterizedReplaceableKind$1(kind) {
  return 3e4 <= kind && kind < 4e4;
}
function classifyKind(kind) {
  if (isRegularKind(kind))
    return "regular";
  if (isReplaceableKind$1(kind))
    return "replaceable";
  if (isEphemeralKind(kind))
    return "ephemeral";
  if (isParameterizedReplaceableKind$1(kind))
...
const GENERIC_TAGS = (LETTERS + LETTERS.toUpperCase()).split("");
...
function getEventUID(event) {
    if (kinds_exports.isReplaceableKind(event.kind) ||
        kinds_exports.isParameterizedReplaceableKind(event.kind)) {
...
const logger = index("nostr-idb");

const log$4 = logger.extend("writeQueue");
class WriteQueue {
    db;
    queuedIds = new Set();
    eventQueue = [];
    ...
    async flush(count = 1000) {
        if (this.eventQueue.length > 0) {
            ...
            if (this.eventQueue.length > 0)
                log$4(`${this.eventQueue.length} left`);
        }
...
const log$3 = logger.extend("cache:indexes");
...
class IndexCache {
    ...
    pruneIndexes() {
        while (this.lastUsed.length > 0 && this.lastUsed.length > this.max) {
            ...
            if (!index)
                return;
            log$3(`Forgetting ${index.type}:${index.key}`);
            switch (index.type) {
                case "kind":
...
const log$2 = logger.extend("prune");
/**
...
async function pruneLastUsed(db, maxEvents, skip) {
    ...
    if (diff <= 0)
        return;
    log$2(`Pruning database to ${maxEvents}`);
...
const log$1 = logger.extend("relay");
...
class RelayCore {
    ...
    async executeSubscription(sub) {
        ...
        log$1(`Running ${sub.id}`, sub.filters);
                ...
                if (sub.onevent) {
                    ...
                    for (const event of events) {
                        try {
                            ...
                            log$1(`onevent failed with error`, error);
                        }
                    }
                    const delta = new Date().valueOf() - start;
                    log$1(`Finished ${sub.id} took ${delta}ms and got ${events.length} events`);
                }
                if (sub.oneose)
                    sub.oneose();
                res();
...
async function sendMessageToRelay(core, message, respond) {
    switch (message[0]) {
        case "EVENT":
            if (message[1]) {
...
function connectRelayToWorkerContext(core, rx, tx) {
    rx((event) => {
...
function connectRelayToMessagePort(core, port) {
